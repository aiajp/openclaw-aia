...
## Claude Code内でCodexを使用する

Claude Code v5.0以降は、公式プラグインを通じてCodexを呼び出すことができる。
これにより、Claude CodeとCodexを協調動作させて複雑なタスクを実行できる。

**詳細なセットアップ手順**: `references/codex-setup-guide.md` を参照

### セットアップ手順

1. **Codexをインストール** (グローバル):
...
※ ChatGPTアカウントまたはOpenAI API keyが必要

3. **Claude Codeにプラグインマーケットプレイスを追加**:
...
Claude Codeセッション内で以下のコマンドが使えるようになる:
...
`scripts/setup-codex.sh` を実行すると、Codexのインストールと設定を自動化できる:

```bash
~/.openclaw/skills/skill-claude-code/scripts/setup-codex.sh
```

このスクリプトは:

1. Codexがインストールされているか確認
2. 未インストールなら `npm install -g @openai/codex` を実行
3. Codexプラグインがインストールされているか確認
4. 未インストールならプラグインのインストール手順を提示

### 使用上の注意

- Codexの使用はChatGPT subscription または OpenAI API key使用量に加算される
- バックグラウンド実行推奨（特にレビューやrescueタスク）
- 長時間タスクの場合は `/codex:status` でステータス確認
- レビューゲート機能（`--enable-review-gate`）は使用量を大きく消費するため注意

### Config設定（オプション）

プロジェクトルートに `.codex/config.toml` を作成すると、デフォルトのモデルや設定を変更できる:
...
- **Codex連携**: セッション開始時にCodexのセットアップ状態を検証する
